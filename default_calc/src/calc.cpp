...
double to_deg(double result){ return result * 180.0 / PI; }
double to_rad(double result){ return result * (PI/180.0); }
...
switch (op) {
      ...
      case Op::CTN:{
            if (rad_on && std::fmod(abs(current), PI)!=0) return 1/std::tan(current);
      ...
      case Op::ACTN:{
            if (rad_on) return PI/2 - std::atan(current);
            else return to_deg((PI/2 - std::atan(current)));
      }
        default:
            return current;
    }
...
// правка табуляции + выбор режима вычислений
